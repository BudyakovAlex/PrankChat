...
namespace PrankChat.Mobile.Core.Presentation.ViewModels.Profile.Cashbox
{
    public class WithdrawalViewModel : BasePageViewModel
    {
        ...
        private string GetWithdrawalAvailableTotalString()
        {
            ...
            return $"{Resources.CashboxViewWithdrawalAvailableTitle} {balance.ToPriceString()}";
        }
        ...
        private async Task WithdrawAsync()
        {
            ...
            try
            {
                if (_currentCard == null)
                {
                    ...
                    if (card == null)
                    {
                        ErrorHandleService.HandleException(new ValidationException(Resources.WithdrawalViewEmptyCardError, ValidationErrorType.CanNotMatch));
                        return;
                    }
                    _currentCard = card;
                    await RaiseAllPropertiesChanged();
        ...
        private async Task CancelWithdrawAsync()
        {
            ...
            {
                 ErrorHandleService.HandleException(new ValidationException(Resources.WithdrawalViewCancelWithdrawalError, ValidationErrorType.CanNotMatch, 0.ToString()));
                return;
            }
        ...
        {
            var result = await UserInteraction.ShowMenuDialogAsync(new string[] { Resources.WithdrawalViewDeleteCardText }, Resources.Close);
            if (result == Resources.WithdrawalViewDeleteCardText)
            {
        ...
        private async Task DeleteCardAsync()
        {
            ...
            try
            {
                var isConfirmed = await UserInteraction.ShowConfirmAsync(Resources.WithdrawalViewDeleteCardQuestion, Resources.Attention, Resources.Delete, Resources.Cancel);
                if (!isConfirmed)
                {
                    return;
                }
