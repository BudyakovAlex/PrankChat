...
{
    public abstract class BaseItemsPageViewModelResult<TItem, TResult> : BasePageViewModelResult<TResult>
    {
        private readonly SerialDisposable _itemsSubscription;

        protected BaseItemsPageViewModelResult()
        {
        ...
        public MvxObservableCollection<TItem> Items
        {
            ...
            protected set
            {
                ...
                if (SetProperty(ref _items, value, OnItemsChanged))
                {
                    ...
                    {
                        await RaisePropertiesChanged(nameof(Count), nameof(IsEmpty), nameof(IsNotEmpty));
                    });
                }
            }
        }
