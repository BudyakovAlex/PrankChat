...
namespace PrankChat.Mobile.Droid.Presentation.Views.Order
{
    ...
    public class OrderDetailsView : BaseView<OrderDetailsViewModel>
    {
        ...
        protected override void SetViewProperties()
        {
            ...
            _uploadingInfoContainerView.SetRoundedCorners(DisplayUtils.DpToPx(30) / 2);
            ...
            _orderDescription = FindViewById<TextView>(Resource.Id.order_descroption_text_view);
            _priceValueTextView = FindViewById<TextView>(Resource.Id.price_value_text_view);
            _orderDetailsViewTimeTextView = FindViewById<TextView>(Resource.Id.orderDetailsView_time_text_view);
            _timeDaysValueLinearLayout = FindViewById<LinearLayout>(Resource.Id.linearLayout_timeDaysValue);
        ...
        protected override void Bind()
        {
            ...
            bindingSet.Bind(_yesSelectableButton)
                      .For(v => v.BindClick())
                      .To(vm => vm.YesCommand);
            bindingSet.Bind(_yesSelectableButton)
            ...
            bindingSet.Bind(_yesSelectableButton)
                      .For(v => v.Text)
                      .To(vm => vm.YesText);
            bindingSet.Bind(_noSelectableButton)
                      .For(v => v.BindClick())
                      .To(vm => vm.NoCommand);
            bindingSet.Bind(_noSelectableButton)
                      .For(v => v.ArbitrationValue)
            ...
            bindingSet.Bind(_noSelectableButton)
                      .For(v => v.Text)
                      .To(vm => vm.NoText);
            bindingSet.Bind(_acceptSelectableButton)
                      .For(v => v.BindClick())
                      .To(vm => vm.AcceptOrderCommand);
            bindingSet.Bind(_acceptSelectableButton)
                      .For(v => v.BindVisible())
                      .To(vm => vm.VideoSectionViewModel.IsDecisionVideoAvailable);
            bindingSet.Bind(_arqueOrderSelectableButton)
                      .For(v => v.BindClick())
                      .To(vm => vm.ArqueOrderCommand);
            bindingSet.Bind(_arqueOrderSelectableButton)
                      .For(v => v.BindVisible())
                      .To(vm => vm.VideoSectionViewModel.IsDecisionVideoAvailable);
            bindingSet.Bind(_animationFrameLayout)
                      .For(v => v.BindVisible())
            ...
            bindingSet.Bind(_profileNameTextView)
                      .For(v => v.Text)
                      .To(vm => vm.CustomerSectionViewModel.ProfileName);
            bindingSet.Bind(_orderTitleTextView)
                      .For(v => v.Text)
                      .To(vm => vm.OrderTitle);
            bindingSet.Bind(_orderDescriptionView)
                      .For(v => v.BindVisible())
                      .To(vm => vm.IsHiddenOrder);
            bindingSet.Bind(_orderDescription)
                      .For(v => v.Text)
                      .To(vm => vm.OrderDescription);
            bindingSet.Bind(_priceValueTextView)
                      .For(v => v.Text)
                      .To(vm => vm.PriceValue);
            bindingSet.Bind(_orderDetailsViewTimeTextView)
                      .For(v => v.BindVisible())
                      .To(vm => vm.IsTimeAvailable);
            bindingSet.Bind(_timeDaysValueLinearLayout)
                      .For(v => v.BindVisible())
                      .To(vm => vm.IsTimeAvailable);
            bindingSet.Bind(_timeDaysTextView)
                      .For(v => v.Text)
                      .To(vm => vm.TimeDaysValue);
            bindingSet.Bind(_timeHoursTextView)
                      .For(v => v.Text)
                      .To(vm => vm.TimeHourValue);
            bindingSet.Bind(_timeMinutesTextView)
                      .For(v => v.Text)
