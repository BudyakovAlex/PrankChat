// WARNING
//
// This file has been generated automatically by Visual Studio to store outlets and
// actions made in the UI designer. If it is removed, they will be lost.
// Manual changes to this file may not be handled correctly.
//
using Foundation;
using System.CodeDom.Compiler;

namespace PrankChat.Mobile.iOS.Presentation.Views.Competition
{
	[Register ("CompetitionDetailsHeaderCell")]
	partial class CompetitionDetailsHeaderCell
	{
		[Outlet]
		FFImageLoading.Cross.MvxCachedImageView backgroundImageView { get; set; }

		[Outlet]
		UIKit.UILabel daysLabel { get; set; }

		[Outlet]
		UIKit.UILabel descriptionLabel { get; set; }

		[Outlet]
		UIKit.UILabel durationLabel { get; set; }

		[Outlet]
		UIKit.UILabel hoursLabel { get; set; }

		[Outlet]
		UIKit.UILabel idLabel { get; set; }

		[Outlet]
		UIKit.UIButton likeButton { get; set; }

		[Outlet]
		UIKit.UILabel minutesLabel { get; set; }

		[Outlet]
		UIKit.UIButton openPrizePoolButton { get; set; }

		[Outlet]
		UIKit.UIButton openRulesButton { get; set; }

		[Outlet]
		UIKit.UILabel prizeLabel { get; set; }

		[Outlet]
		UIKit.UILabel prizeTitleLabel { get; set; }

		[Outlet]
		UIKit.UILabel termLabel { get; set; }

		[Outlet]
		UIKit.UIView timeContainer { get; set; }

		[Outlet]
		UIKit.UILabel timeLabel { get; set; }

		[Outlet]
		UIKit.UILabel titleLabel { get; set; }

		[Outlet]
		UIKit.UIButton uploadVideoButton { get; set; }
		
		void ReleaseDesignerOutlets ()
		{
			if (backgroundImageView != null) {
				backgroundImageView.Dispose ();
				backgroundImageView = null;
			}

			if (daysLabel != null) {
				daysLabel.Dispose ();
				daysLabel = null;
			}

			if (descriptionLabel != null) {
				descriptionLabel.Dispose ();
				descriptionLabel = null;
			}

			if (durationLabel != null) {
				durationLabel.Dispose ();
				durationLabel = null;
			}

			if (hoursLabel != null) {
				hoursLabel.Dispose ();
				hoursLabel = null;
			}

			if (idLabel != null) {
				idLabel.Dispose ();
				idLabel = null;
			}

			if (likeButton != null) {
				likeButton.Dispose ();
				likeButton = null;
			}

			if (minutesLabel != null) {
				minutesLabel.Dispose ();
				minutesLabel = null;
			}

			if (openPrizePoolButton != null) {
				openPrizePoolButton.Dispose ();
				openPrizePoolButton = null;
			}

			if (openRulesButton != null) {
				openRulesButton.Dispose ();
				openRulesButton = null;
			}

			if (prizeLabel != null) {
				prizeLabel.Dispose ();
				prizeLabel = null;
			}

			if (prizeTitleLabel != null) {
				prizeTitleLabel.Dispose ();
				prizeTitleLabel = null;
			}

			if (termLabel != null) {
				termLabel.Dispose ();
				termLabel = null;
			}

			if (timeContainer != null) {
				timeContainer.Dispose ();
				timeContainer = null;
			}

			if (timeLabel != null) {
				timeLabel.Dispose ();
				timeLabel = null;
			}

			if (titleLabel != null) {
				titleLabel.Dispose ();
				titleLabel = null;
			}

			if (uploadVideoButton != null) {
				uploadVideoButton.Dispose ();
				uploadVideoButton = null;
			}
		}
	}
}
